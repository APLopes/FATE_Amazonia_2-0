...
## for forests only
## 
## Mapbiomas c2 x MCD64 burned area 
## SM figure
##
...
##
## called 'SEEGFire_AM_Manuscript_Emissions_MCD64_SupFigure_20230515.R' in my PC
##_____________________________________________________________________________
...
path = './Data/MCD64_comparison/Mapbiomas_Fire_col2_2001to2022_legacy'
...
path = './Data/MCD64_comparison/MCD64_2001to2022_legacy'
...
df2.CO2eq.sum.cum = df2.CO2eq.sum %>% 
  dplyr::arrange(Year) %>%
  group_by(Base) %>%
  dplyr::mutate(Emissions_Tg_Co2eq_cum = cumsum(Emissions_Tg_Co2eq)) %>%
  dplyr::arrange(Base)
...
#export table
write.xlsx(df2.CO2eq.sum,
           file = "./Tables/MBc2_MCD64_CO2eq_emissions_2002-2022.xlsx", 
           overwrite = TRUE)
