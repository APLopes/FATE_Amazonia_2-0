...
# "SEEGFire_SA_1.3_Model_output_Multiburn_ManuscriptFigure_20220525.R"
load('SEEGmodel_20220525.Rdata')
...
png("./figures/SEEGFire_AM_IntModel_SA_Turnover_timelines_20230519.png",
...
scen_TurnOver_Decomp_summary = 
  scen_TurnOver_Decomp[,c("time","CBalance")] %>% 
  group_by(time) %>%
  summarize(mean.CB = mean(CBalance),
            sd.CB = sd(CBalance),
            n.CB = n()) %>%
  mutate(upper.sd.CB = mean.CB + sd.CB,
         lower.sd.CB = mean.CB - sd.CB,
...
scen_TurnOver_Decomp_total = scen_TurnOver_Decomp %>%
  group_by(TOver.rate, Decomp.rate) %>%
  summarise(total = sum(CBalance))
...
png("./figures/ManuscriptFigureSM6_SA_1fires_20230519.png",
    width = 20, height = 8, units = 'cm', res = 400)
