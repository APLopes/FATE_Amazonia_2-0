...
//cria imagem de 35 bandas com floresta/nao-floresta (1,0)
  ...
  var grid = ee.FeatureCollection('projects/ee-ipam/assets/SEEG/SEEG13/10grid')
    .map(function(feature){
      ...
      var ysf_std_for_area = ee.Image.pixelArea().divide(1e6).updateMask(all_ysf_std_for_area.gte(1))
      .reduceRegion({
        reducer:ee.Reducer.sum(),
        geometry:feature.geometry(),
        scale:30, 
      ...
      return feature.set({
        ysf_desf:ysf_desf_area,
        ysf_std_for:ysf_std_for_area,
      });
    });
  
  print('grid',grid.limit(10));
  
  Export.table.toAsset({
    collection:grid,
    description:'SEEG-grid-ysf_forest_stable-and-ysf_deforestation',
    assetId:'projects/ee-seegfiredyn/assets/grid-ysf_forest_stable-and-ysf_deforestation',
    // maxVertices:
  });
