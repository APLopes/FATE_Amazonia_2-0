...
##
## APL: version called "SEEGFire_AM_Manuscript_Emissions_MainFigure_20231015.R" 
...
png("./Figures/SM_Figure6.png", width = 20, height = 25, units = 'cm', res = 400) 
ps
dev.off()
...
df.query2 = droplevels(subset(df.main.fig, Year.num >= 2015 & Year.num <= 2022))
df.query2 = df.query2 %>%
  group_by(Year) %>%
  summarise(Emissions_Tg_Co2eq = sum(Emissions_Tg_Co2eq))

round(mean(df.query2$Emissions_Tg_Co2eq),1) #83.5
round(sd(df.query2$Emissions_Tg_Co2eq),1) #19.7
...
# saving data ------------------------------------------------------------------
# export to compare with 2002-2020 models based on 
# the Mapbiomas Fire and MCD64 burned area maps
# save(list = c('df', 'df.main.fig', 'df.ss'), 
#      file='./Model_outputs_1986_2022_with_legacy_20230426.Rdata')
