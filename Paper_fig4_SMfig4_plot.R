...
##
## Made by: Aline Pontes Lopes, 30/07/2021
## last edit: 26/04/2023
##
## APL: version called "SEEGFire_AM_Manuscript_Emissions_MainFigure_20230426.R" in my PC
...
path = 'C:/Users/Usuario/OneDrive - inpe.br/IPAM/GEE-sheets/GEE_FireDyn_IntegratedModel_AM_output_paper_v2'
...
df.ss$Year.num = as.numeric(df.ss$Year)
df.ss$Year = as.factor(df.ss$Year)
df.ss$legacy = ifelse(df.ss$Year.num > 2022, 1, 0)
df.ss$legacy = as.factor(df.ss$legacy)
df.ss = droplevels(df.ss)
...
ps = ggplot(df.ss) +
  ...
  geom_text(data=df.text, y=78, aes(x=Year, label=label),
...
png("./figures/Manuscript_Sup_Figure_Each_gas_Tg_20230426.png", width = 20, height = 25, units = 'cm', res = 400) 
ps
dev.off()
...
# used AR6 100-y GWP 
df.main.fig$Emissions_Tg_Co2eq = ifelse(df.main.fig$GHG == 'CH4', df.main.fig$Emissions_Tg*27.2, df.main.fig$Emissions_Tg)
df.main.fig$Emissions_Tg_Co2eq = ifelse(df.main.fig$GHG == 'N2O', df.main.fig$Emissions_Tg*273, df.main.fig$Emissions_Tg_Co2eq)
...
png("./figures/Manuscript_Figure5_Main_gases_Tg_20230426.png", width = 20, height = 10, units = 'cm', res = 400)
pm
dev.off()
...
sum(subset(df.main.fig, Year.num >=1990 & Year.num<=2022)$Emissions_Tg_Co2eq) # 1840.239 Tg

# CO2eq emissions in 2007
sum(subset(df.main.fig, Year.num == 2007)$Emissions_Tg_Co2eq) # 87.9 Tg year-1

# Co2eq emissions since 2015
...
# legacy emissions
sum(subset(df.main.fig, Year.num >= 2022 )$Emissions_Tg_Co2eq) # 693,6 Tg year-1


# saving data ------------------------------------------------------------------
# export to compare with 2002-2020 models based on the Mapbiomas Fire and MCD64 burned area maps
save(list = c('df', 'df.main.fig', 'df.ss'), file='./Model_outputs_1986_2022_with_legacy_20230426.Rdata')
